...
{panel}`;
  ...
  static toJ(input, settings) {
    ...
    input = input.replace(/`{3,}(\w+)?((?:\n|.)+?)`{3,}/g, function(_, synt, content) {
      ...
      replacementsList.push({ key, value: code2 });
      return key;
    });
    ...
    input = input.replace(new RegExp("<(" + Object.keys(map3).join("|") + ")>(.*?)</\\1>", "g"), function(_, from, content) {
      const to = map3[from];
      return to + content + to;
    });
...
var MTJSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Use Legacy Converter").setDesc("We are in the process of developing an enhanced converter that aims to support all previous formatting options. However, there might still be some features that are not fully functional. If you encounter any issues, you can use this toggle to switch back to the previous converter.").addToggle((toggle) => toggle.setValue(this.plugin.settings.useLegacyConverter).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Render metadata").setDesc("Transforms the metadata at the top of an Obsidian note, enclosed by ---, into a Jira-compatible format, resulting in a list of key-value pairs labeled as 'Metadata'. Only works with the new converter!").addToggle((toggle) => toggle.setValue(this.plugin.settings.renderMetadata).setDisabled(this.plugin.settings.useLegacyConverter).onChange(async (value) => {
      this.plugin.settings.renderMetadata = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    containerEl.createEl("h2", { text: "Images" });
    new import_obsidian.Setting(containerEl).setName("Image Translation (still  in development").setDesc("Should images be uploaded to one of the given hosts?").addToggle((toggle) => {
      toggle.setDisabled(true);
      toggle.setValue(this.plugin.settings.imageUpload.enabled);
    });
    new import_obsidian.Setting(containerEl).setName("Image Upload Hoster").setDesc("Choose one of the provided image upload hoster. You have to get the credentials or the api key of your own!").addDropdown((dropdown) => {
      dropdown.addOption("https://api.imgur.com/", "Imgur").setDisabled(!this.plugin.settings.imageUpload.enabled).setValue(this.plugin.settings.imageUpload.host).onChange(async (value) => {
        this.plugin.settings.imageUpload.host = value;
        await this.plugin.saveSettings();
        this.display();
      });
    });
    new import_obsidian.Setting(containerEl).setName("API Key").addText((text2) => text2.setValue(this.plugin.settings.imageUpload.apiKey || "").setDisabled(!this.plugin.settings.imageUpload.enabled).onChange(async (value) => {
    ...
    for (const [i, val] of settings.calloutConfigurations.entries()) {
      ...
      new import_obsidian.Setting(containerEl).setName("Choose title icon").addDropdown((dropdown) => {
        dropdown.addOptions(Object.entries(calloutIcons).reduce((acc, [key, { jiraTag, displayName }]) => ({
          ...acc,
          [jiraTag]: displayName
...
function getDecoder(decodeTree) {
  ...
  return function decodeWithTrie(str2, decodeMode) {
    ...
    while ((offset = str2.indexOf("&", offset)) >= 0) {
      ...
      const len = decoder.write(str2, offset + 1);
      if (len < 0) {
...
var getCodePoint = String.prototype.codePointAt != null ? (str2, index) => str2.codePointAt(index) : (c, index) => (c.charCodeAt(index) & 64512) === 55296 ? (c.charCodeAt(index) - 55296) * 1024 + c.charCodeAt(index + 1) - 56320 + 65536 : c.charCodeAt(index);
...
function callouts(md, options) {
  md.block.ruler.before("blockquote", "callout", (state, startLine, endLine, silent) => {
    ...
    tokenOpen.content = marker[1].toLowerCase();
    tokenOpen.info = marker[2];
...
function extractFrontmatter(markdown) {
  ...
  const match2 = markdown.match(frontmatterRegex);
  if (match2) {
    return load(match2[1]);
  }
  return null;
}

// src/rules/wikiLinks.ts
function wikiLinks(md) {
  md.inline.ruler.before("link", "wikilink", (state, silent) => {
    const max = state.posMax;
    const start = state.pos;
    if (state.src.charCodeAt(start) !== 91 || state.src.charCodeAt(start + 1) !== 91) {
      return false;
    }
    let pos = start + 2;
    while (pos < max) {
      if (state.src.charCodeAt(pos) === 93 && state.src.charCodeAt(pos + 1) === 93) {
        break;
      }
      pos++;
    }
...
var Translator = class {
  ...
  preserveLineBreaks(markdown) {
    return markdown.replace(/\n/g, "<LINEBREAK>");
  }
  postProcessOutput(renered) {
    return renered.replace(/\<LINEBREAK\>/g, "\n");
  }
  convertMarkdownToJira(markdown) {
    const frontmatter = extractFrontmatter(markdown);
    const contentWithoutFrontmatter = markdown.replace(/^---\n[\s\S]*?\n---/, "");
    const md = new lib_default().use(basics).use(wikiLinks).use(callouts, this.plugin.settings.calloutConfigurations);
...
var UpdateModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    if (this.version == "0.2.0") {
      new import_obsidian2.Setting(contentEl).setName("Thank you for using this plugin.").setDesc("I'm currently working on a significantly improved converter that will be much easier to extend. Stay tuned for exciting new features and enhancements!");
      new import_obsidian2.Setting(contentEl).setName("").setDesc("With this update, the new converter has been enabled by default. Since it's in an early beta stage, I've added an option in the settings to switch back to the legacy converter if needed.");
      new import_obsidian2.Setting(contentEl).setName("Stay tuned; more exciting updates are coming soon!");
    }
  }
};

// src/main.ts
var MTJPlugin = class extends import_obsidian3.Plugin {
  async onload() {
